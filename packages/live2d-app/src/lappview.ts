...
import { CubismViewMatrix } from "@ai-zen/live2d-framework/dist/math/cubismviewmatrix";
...
export class LAppView {
  ...
  public onTouchesBegan(pointX: number, pointY: number): void {
    ...
    this._touchBeginTime = Date.now(); // 接触开始时记录时间
  }
  ...
  public onTouchesEnded(pointX: number, pointY: number): void {
    ...
    {
      // 如果接触时长大于 LAppDefine.TouchTapMaxDuration 则判断为非点击，直接返回
      if (Date.now() - this._touchBeginTime > LAppDefine.TouchTapMaxDuration)
        return;
  ...
  _touchBeginTime: number; // 接触开始时的时间
}
